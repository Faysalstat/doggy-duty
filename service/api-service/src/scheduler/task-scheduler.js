...
exports.generateInvoice = async () => {
  ...
  try {
    let communities = await Community.findAll({include: [{ model: CommunityServiceSchedule }]});
    for (let i = 0; i < communities.length; i++) {
      let community = communities[i].dataValues;
      console.log(community);
      if (community.communityServiceSchedule?.lastInvoiceGenerated) {
        const lastGenerated = moment(community.communityServiceSchedule.lastInvoiceGenerated);
        ...
        if (today.diff(lastGenerated, "days") === 30) {
          ...
          let bills = await Billing.findAll({where:{invoiceGenerated:false,communityId:community.id},include:Task});
          if(bills && bills.length>0){
            ...
            for (let j = 0; j < bills.length; j++) {
              ...
              let createdMapping = await InvoiceBillMapping.create(invoiceBillMappingModel);
            }
          }
          

          // Update lastInvoiceGenerated to today
          community.communityServiceSchedule.lastInvoiceGenerated = today.toDate();
          await CommunityServiceSchedule.update({lastInvoiceGenerated:today.toDate()},{where:{id:community.id}});
        }
      }
    }
  } catch (error) {
